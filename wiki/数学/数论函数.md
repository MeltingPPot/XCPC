...
$$
\begin{aligned}
    F(s)G(s)&=F(s)\prod_p(1+\frac{g(p)}{p^s}+\frac{g(p^2)}{p^{2s}}+\cdots)
\end{aligned}
$$
...
根据：$\lfloor\frac{n}{i}\rfloor=\lceil\frac{n-1}{i}\rceil+1$（自证不难），可以将其转换成向下取整的形式：
...
\begin{cases}
1,	n=1,\\[4pt]
0,	n\text{ is divisible by a square}xcz
\\[4pt]
(−1)^k,	n \text{ is the product of } k \text{ distinct primes.}
\end{cases}
$$
...
$$
\begin{aligned}
    \sum_{n=1}^\infty\frac{\mu(n)}{n^s}&=\prod_{p}(1+\frac{-1}{p^s}+\frac{0}{p^{2s}}+\cdots)\\[4pt]
    &=\prod_p(1-p^{-s})\\[4pt]
    &=\frac{1}{\zeta(s)}\\[4pt]
\end{aligned}
$$
...
$$
\begin{aligned}
    f(n)&=\sum_{d\vert n} g(d)\\[4pt]
    g(n)&=\sum_{d\vert n}\mu(d)f(d)
\end{aligned}
$$
...
$$
\begin{aligned}
    \sum_{d\vert n} \sigma_1(d)\varphi(\frac{n}{d})=n\sigma_0(n)
\end{aligned}
$$
...
$$
\begin{align*}
\sum_{d\mid n}\sigma_1(d)\,\varphi\!\left(\frac{n}{d}\right)
  &= \sum_{d\mid n}\Bigl(\sum_{a\mid d}a\Bigr)\,\varphi\!\left(\frac{n}{d}\right)\\[4pt]
  &= \sum_{a\mid n}a\!\!\sum_{a\mid d\mid n}\varphi\!\left(\frac{n}{d}\right) \\[4pt]
  &= \sum_{a\mid n}a\!\!\sum_{b\mid n/a}\varphi\!\left(\frac{n}{ab}\right)
      && (d=ab) \\[2pt]
  &= \sum_{a\mid n}a\cdot\frac{n}{a}
      && \Bigl(\textstyle\sum_{c\mid n/a}\varphi(c)=\frac{n}{a}\Bigr) \\[2pt]
  &= n\,\sigma_0(n)
\end{align*}
$$
...
- $n^{\prime}\bmod{p}=0$ 此时若 $p^c\parallel n^{\prime}$，则 
  ...
  $$\begin{aligned}
  \varphi(n)&=\varphi\left(\frac{n^{\prime}}{p^c}\right)\varphi(p^{c+1})\\
  &=\frac{\varphi(n^{\prime})}{\varphi(p^c)}\varphi(p^{c+1})\\
  &=\frac{\varphi(n^{\prime})}{p^{c-1}(p-1)}p^{c}(p-1)\\
  &=\varphi(n^{\prime})p\\
  \end{aligned}
  $$
...
\sigma_0(n)=
\begin{cases}
    \frac{\sigma_0(n^{\prime})}{c+1}(c+2),&p\mid n^{\prime}\\
    2\sigma_0(n^{\prime}),&p \nmid n^{\prime}
\end{cases}
$$
...
\sigma_1(n)=
\begin{cases}
    \frac{\sigma_1(n^{\prime})}{g(p)}(g(p)+p^{k+1}),&p\mid n^{\prime}\\
    \sigma_1(n^{\prime})(p+1),&p \nmid n^{\prime}
\end{cases}
$$
